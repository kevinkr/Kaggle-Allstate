...
#########################
#Examine continuous correlations
##########################
# 1. Examine strong correlations
# 2. 
#############################
...
pairwiseCor(train.num)
#outliers
pairwiseCor(notoutliers.num)
pairwiseCor(outliers.num)

##### Drop 1, 11, 6,10
train.num <- subset(train.num, select =-c(cont1,cont6,cont11,cont10))
notoutliers.num <- subset(notoutliers.num, select =-c(cont1,cont6,cont11,cont10))
outliers.num <- subset(outliers.num, select =-c(cont1,cont6,cont11,cont10))
pairwiseCor(train.num)
#outliers
pairwiseCor(notoutliers.num)
pairwiseCor(outliers.num)

plot(train.num$cont3,train.num$cont5)
# factor analysis - see http://rtutorialseries.blogspot.com/2011/10/r-tutorial-series-exploratory-factor.html


# Multicollinearity - See http://stats.stackexchange.com/questions/87608/confused-about-multicollinearity-variable-selection-and-interaction-terms
#
library(perturb)

m1 <- lm(log(loss) ~ cont1 + cont2 + cont3 + cont4 + cont5 + 
           cont6 + cont7 + cont8 + cont9 + cont10 + cont11 + 
           cont12 + cont13 + cont14, data = train)

cd<-colldiag(m1)
cd
# 	
# It doesn't tell you what's correlated with what (you could get that from a 
# correlation matrix) it tells you what variables are contributing to the 
# collinearity. Read each row separately. Look for rows with large condition 
# indexes (above 10 or above 30, depending on who you ask). Then, in each row, 
# look for variables that have a high proportion of contribution (some people 
# say above .5, some above .4).

# There are various things to do when you have collinearity: Delete variables; 
# use ridge regression; get more data; use partial least squares are 4 that come 
# to mind 

##### Drop 1, 11, 6,10
m1 <- lm(loss ~ cont2 + cont3 + cont4 + cont5 + 
           cont7 + cont8 + cont9 + 
           cont12 + cont13 + cont14, data = train)
cd<-colldiag(m1)
cd
